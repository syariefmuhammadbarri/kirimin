...
<div class="max-w-xl mx-auto">
    ...
    <div class="mb-7">
        <h1 class="text-2xl font-bold text-slate-900">Edit Cabang</h1>
        <p class="text-sm text-slate-500 mt-1">{{ $branch->name }}</p>
    </div>
        ...
        <form method="POST" action="{{ route('manager.branches.update', $branch) }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="address" name="address" rows="3" required
                          class="w-full px-4 py-3 rounded-lg bg-white border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition text-sm resize-none">{{ old('address', $branch->address) }}</textarea>
