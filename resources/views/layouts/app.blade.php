...
<body class="min-h-screen flex flex-col" data-lenis-prevent>
    <!-- Navbar / Header -->
    <header class="sticky top-0 z-40 bg-white/90 backdrop-blur-lg border-b border-black/5">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <!-- Logo -->
                <div class="flex items-center">
                    <a href="{{ route('landing') }}" class="flex items-center space-x-2">
                        <span class="text-xl font-bold tracking-wider text-blue-600 uppercase">BAZMA</span>
                        <span class="text-xl font-semibold text-slate-700 uppercase">Express</span>
                    </a>
                </div>
                ...
                <div class="flex items-center space-x-4">
                    @auth
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="text-xs bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 px-3 py-1.5 rounded transition font-medium">
                                Keluar
                            </button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="text-sm font-medium text-slate-600 hover:text-slate-900 transition">Masuk</a>
                        <a href="{{ route('register') }}" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-4 py-2 rounded transition shadow-sm"> Daftar </a>
                    @endauth
                </div>
            </div>
        </div>
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <!-- Toast Alerts -->
        @if (session('success'))
            <div class="mb-6 p-4 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 flex items-center space-x-3" x-data="{ show: true }" x-show="show" x-transition>
        ...
        @if (session('error'))
            <div class="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-red-700 flex items-center space-x-3" x-data="{ show: true }" x-show="show" x-transition>
    ...
    <!-- Footer -->
    <footer class="border-t border-black/5 py-6 text-center text-xs text-slate-400 mt-auto bg-white">
    ...
    <!-- Lenis Smooth Scroll (loaded via JS) -->
