...
<div class="max-w-xl mx-auto">
    ...
    <h1 class="text-2xl font-bold text-white mb-6">Tambah Konten Landing Page</h1>

    <form method="POST" action="{{ route('manager.landing-contents.store') }}" class="glass-panel rounded-2xl border border-slate-800 p-6 space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input id="order" type="number" name="order" value="{{ old('order', 0) }}" min="0" required
                       class="w-full px-4 py-3 rounded-lg bg-slate-900/60 border border-slate-700/80 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-sm">
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="5"
                      class="w-full px-4 py-3 rounded-lg bg-slate-900/60 border border-slate-700/80 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-sm resize-none"
                      placeholder="Isi konten...">{{ old('content') }}</textarea>
        ...
        <div>
            ...
            <input id="image" type="text" name="image" value="{{ old('image') }}" maxlength="500"
                   class="w-full px-4 py-3 rounded-lg bg-slate-900/60 border border-slate-700/80 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-sm"
