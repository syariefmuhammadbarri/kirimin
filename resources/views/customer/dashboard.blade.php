...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-white">Dashboard Pelanggan</h1>
        <p class="text-sm text-slate-400 mt-1">Selamat datang kembali, <span class="text-blue-400 font-medium">{{ $customer->name ?? Auth::user()->name }}</span></p>
    </div>
    <a href="{{ route('customer.booking.create') }}"
       class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold px-5 py-2.5 rounded-lg shadow-lg shadow-blue-950/50 transition duration-150">
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    <div class="glass-panel rounded-xl p-5 border border-slate-800">
        <p class="text-xs text-slate-400 uppercase tracking-wider mb-1">Total Pengiriman</p>
        <p class="text-3xl font-bold text-white">{{ $totalShipments }}</p>
    </div>
    <div class="glass-panel rounded-xl p-5 border border-yellow-900/30">
        <p class="text-xs text-yellow-500/80 uppercase tracking-wider mb-1">Menunggu Bayar</p>
        <p class="text-3xl font-bold text-yellow-400">{{ $pendingPayment }}</p>
    </div>
    <div class="glass-panel rounded-xl p-5 border border-blue-900/30">
        <p class="text-xs text-blue-500/80 uppercase tracking-wider mb-1">Dalam Proses</p>
        <p class="text-3xl font-bold text-blue-400">{{ $inProgress }}</p>
    </div>
    <div class="glass-panel rounded-xl p-5 border border-emerald-900/30">
        <p class="text-xs text-emerald-500/80 uppercase tracking-wider mb-1">Terkirim</p>
        <p class="text-3xl font-bold text-emerald-400">{{ $delivered }}</p>
    </div>
</div>
...
<div class="glass-panel rounded-2xl border border-slate-800 overflow-hidden">
    <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
        <h2 class="text-base font-semibold text-white">Riwayat Pengiriman</h2>
        <span class="text-xs text-slate-500">{{ $totalShipments }} paket</span>
    </div>
    ...
    @if($shipments->isEmpty())
        <div class="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p class="text-slate-400 font-medium">Belum ada pengiriman</p>
            <p class="text-slate-600 text-sm mt-1">Buat pengiriman pertama Anda sekarang</p>
            <a href="{{ route('customer.booking.create') }}" class="mt-4 text-sm text-blue-400 hover:text-blue-300 font-medium transition">Buat Pengiriman &rarr;</a>
        </div>
    @else
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-800/50">
                    ...
                    <tr class="hover:bg-slate-800/20 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-slate-200 font-medium">{{ $shipment->origin_city }}</div>
                        ...
                        <td class="px-6 py-4">
                            <span class="text-xs font-semibold uppercase px-2 py-1 rounded 
                                {{ $shipment->service_type === 'express' ? 'bg-amber-950/50 text-amber-400 border border-amber-900/40' : 'bg-slate-800 text-slate-400 border border-slate-700' }}">
                                {{ $shipment->service_type }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <span class="font-semibold text-white">Rp {{ number_format($shipment->total_price, 0, ',', '.') }}</span>
                            @if($shipment->payment)
                            ...
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @if($shipment->payment && $shipment->payment->payment_status !== 'paid')
                                    <button onclick="openPaymentModal({{ $shipment->id }}, '{{ $shipment->tracking_number }}')"
                                            class="text-xs bg-blue-600/80 hover:bg-blue-600 text-white px-3 py-1.5 rounded transition font-medium">
                                        Bayar
                                    </button>
                                @endif
...
<div id="payment-modal" class="fixed inset-0 z-50 hidden" aria-modal="true" role="dialog">
    ...
    <div class="relative flex items-center justify-center min-h-screen p-4">
        <div class="glass-panel w-full max-w-md rounded-2xl border border-slate-700 shadow-2xl p-6 relative z-10">
            <h3 class="text-lg font-bold text-white mb-1">Pembayaran Pengiriman</h3>
            <p id="modal-tracking" class="text-sm text-slate-400 mb-6 font-mono"></p>

            <div class="space-y-3 mb-6">
                <p class="text-sm text-slate-300">Pilih metode simulasi pembayaran:</p>
                <div class="glass-panel rounded-lg border border-slate-700 p-4 text-center">
                    <p class="text-xs text-slate-400 mb-1">Midtrans Payment Gateway</p>
                    <p class="text-sm text-slate-300">Di produksi nyata, widget Midtrans Snap akan tampil di sini.</p>
                </div>
            </div>
            ...
            <button onclick="closePaymentModal()" class="w-full mt-3 py-2.5 text-sm text-slate-400 hover:text-white transition">
                Batal
            </button>
        </div>
    </div>
